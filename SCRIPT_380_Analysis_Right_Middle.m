...
for i = 1:1:length(fileNames)
    ...
    %When Cropping
    ImSurf_Right(i).Data = im2gray(imcrop(Images(i).Data,rectangle));% Turn to Gray
    
    %No Cropping for April Tag
    ImSurf(i).Data = im2gray(Images(i).Data);% Turn to Gray
    
    BW1(i).Data = edge(ImSurf_Right(i).Data,'sobel',[],'horizontal'); % Detect Edges
    ...
    hold on
    plot(x_plot+1800, y_plot, '-m','Linewidth',2);
end
