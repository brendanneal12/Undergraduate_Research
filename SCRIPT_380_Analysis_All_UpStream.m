...
for i = 1:1:length(file_names)
    ...
    if save_data==1
        UpStreamData.polynomial.Coeffs(count,1).frame = i;
        UpStreamData.polynomial.Time(count,1).Time = i*TimePerFrame;
        UpStreamData.polynomial.Coeffs(count,:).data = p; % store polynomial coefficients here
        
        UpStreamData.polyProcessed(count).frame = i;
        UpStreamData.polyProcessed(count).Time = i*TimePerFrame;
        UpStreamData.polyProcessed(count).points = [x_plot ; y_plot; ones(1,length(x_plot))]; %store fitted points here.
    end
...
if save_data == 1
    save('UpStreamData.mat', 'UpStreamData') %Save the specified data to a variable.
end
